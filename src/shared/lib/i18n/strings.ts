export type Locale = 'en' | 'ru';

export const STRINGS: Record<Locale, Record<string, string>> = {
  en: {
    title: 'Mimo Profile Widget',
    subtitle: 'Streak, XP and coins - live, in your README.',
    emailLabel: 'Email',
    passwordLabel: 'Password',
    howItWorks: 'How it works',
    step1: 'Password → sent once, over HTTPS',
    step2: 'Discarded → only an encrypted refresh token is kept',
    step3: 'Public URL → holds no secrets, decrypts server-side on each view',
    step4: 'Disconnect anytime to delete the stored token',
    connect: 'Connect account',
    connecting: 'Connecting…',
    theme: 'Theme',
    statsToShow: 'Stats',
    readmePreview: 'How it will look on your GitHub profile',
    preloadingTitle: 'Warming up your widget',
    preloadingSubtitle: 'Pre-loading every theme and stat combination so switching feels instant',
    mockName: 'Your Name',
    mockUsername: 'username',
    editProfile: 'Edit profile',
    mockFollowers: '128 followers · 12 following',
    mockLocation: 'Your city, Country',
    imageUrl: 'Image URL',
    readmeMarkdown: 'README markdown',
    copy: 'Copy',
    copied: 'Copied',
    disconnect: 'Disconnect',
    statStreak: 'Streak',
    statCoins: 'Coins',
    statXp: 'XP',
  },
  ru: {
    title: 'Mimo Profile Widget',
    subtitle: 'Стрик, XP и монеты - живые, прямо в README.',
    emailLabel: 'Email',
    passwordLabel: 'Пароль',
    howItWorks: 'Как это работает',
    step1: 'Пароль → отправляется один раз, по HTTPS',
    step2: 'Удаляется → остаётся только зашифрованный refresh-токен',
    step3: 'Публичная ссылка → без секретов, расшифровка на сервере при каждом просмотре',
    step4: 'Можно отключить в любой момент — токен удалится',
    connect: 'Подключить',
    connecting: 'Подключаем…',
    theme: 'Тема',
    statsToShow: 'Статы',
    readmePreview: 'Как это будет выглядеть в вашем профиле GitHub',
    preloadingTitle: 'Готовим виджет',
    preloadingSubtitle: 'Загружаем все темы и комбинации статов, чтобы переключение было мгновенным',
    mockName: 'Ваше имя',
    mockUsername: 'username',
    editProfile: 'Edit profile',
    mockFollowers: '128 followers · 12 following',
    mockLocation: 'Ваш город, страна',
    imageUrl: 'Ссылка',
    readmeMarkdown: 'Markdown',
    copy: 'Копировать',
    copied: 'Скопировано',
    disconnect: 'Отключить',
    statStreak: 'Стрик',
    statCoins: 'Монеты',
    statXp: 'XP',
  },
};
